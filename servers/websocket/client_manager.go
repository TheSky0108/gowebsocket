...
type ClientManager struct {
	Clients     map[*Client]bool   // 全部的连接；记录全部的连接、登录用户的可以通过 appId+uuid 查到用户连接
	ClientsLock sync.RWMutex       // 读写锁；使用map存储，涉及到多协程并发读写的问题，所以需要加读写锁
...
// 获取用户所在的连接
func GetToUserClient(appId uint32, toId string) (client *Client) {
	client = clientManager.GetUserClient(appId, toId)
	return
}
